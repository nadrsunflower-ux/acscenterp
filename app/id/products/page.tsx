...
// 악센트 아이디 - 운영상품 및 환불 (운영 상품 + 환불 안내 묶음).
...
const FALLBACK_PRODUCTS = defaultProducts.filter((p) => p.store === "id");

export default function IdProductsPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const prods = await listProducts("id");
        if (!alive) return;
