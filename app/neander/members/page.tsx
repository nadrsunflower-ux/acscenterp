...
function MemberCard({ member, isMe }: { member: Member; isMe: boolean }) {
  ...
  }, [member.name, member.role, member.email, member.color, editing]);
  ...
  const avatarColor = member.color ?? "#71717a";
  const initial = member.name.trim().charAt(0) || "?";

  // ---- 편집 모드 ----
  if (editing) {
    return (
      <Card className="flex w-64 flex-col gap-3">
  ...
  // ---- 표시 모드 (캐릭터 카드) ----
  return (
    <Card className="flex w-44 flex-col items-center gap-2 text-center">
      <div
