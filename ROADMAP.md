...
- [ ] **실데이터 적재** — 마스터 탭에서 적재 → 임포트 탭에서 2605·2606·2607 장부 올리기
- [ ] `firebase:deploy:rules` 로 `neander_fin_*` 규칙 게시 (안 하면 운영에서 권한 오류)
- [ ] 2605·2606 은 **구스키마**(28열)라 구→신 분류 매핑이 필요할 수 있음 — 임포트 시 오류 행 확인
- [ ] 재무 전용 역할 분리 (아래 Phase 3) — 현재는 팀원이면 누구나 회사 재무 원장을 본다
